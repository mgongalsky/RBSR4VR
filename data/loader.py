...
def collate_stack1(batch):
    ...
    if isinstance(batch[0], torch.Tensor):
        ...
        return {key: collate_stack1([d[key] for d in batch]) for key in batch[0]}
    elif isinstance(batch[0], collections.Sequence):
        transposed = zip(*batch)
        return [collate_stack1(samples) for samples in transposed]
    elif batch[0] is None:
        return batch
