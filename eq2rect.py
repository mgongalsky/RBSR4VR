...
class NFOV():
    ...
    def _bilinear_interpolation(self, screen_coord, center_point):
    ...
    def toNFOV(self, frame, center_point):
        self.frame = frame
        # Проверка на наличие альфа-канала (RGBA) и его удаление
        #if self.frame.shape[2] == 4:  # RGBA
        #    self.frame = self.frame[:, :, :3]  # Отбрасываем альфа-канал
        #    self.frame_channel = self.frame.shape[2]
        ...
        return self._bilinear_interpolation(spericalCoord, center_point)


# test the class
if __name__ == '__main__':
    import imageio as im
    img = im.imread('vr_equi_test.png', pilmode='RGB')
    nfov = NFOV()
    center_point = np.array([0.1, .9])  # camera center point (valid range [0,1])
